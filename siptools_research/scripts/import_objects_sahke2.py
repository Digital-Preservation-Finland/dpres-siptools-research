# coding=utf-8
"""Creates PREMIS file metadata for objects by reading SÄHKE2 data
...
S2_NS = {'s2': 'http://www.arkisto.fi/skeemat/Sahke2/2011/12/20'}
...
def parse_arguments(arguments):
    ...
    parser.add_argument('path', type=str, help='Location of sahke2 package')
    parser.add_argument('--sahke2', dest='sahke2', help='Descriptive metadata')
...
def create_premis_object(digital_object, filepath, filename, creation_date,
...
def main(arguments=None):
    ...
    s2_name = args.sahke2
    s2_path = os.path.join(package_path, s2_name)

    tree = ET.parse(s2_path)
    root = tree.getroot()

    for elem in root.xpath('//s2:Document', namespaces=S2_NS):
        filename = elem.xpath('./s2:File/s2:Path',
                              namespaces=S2_NS)[0].text.replace('\\', '/')
        filepath = os.path.join(package_path, filename)
        hashalgorithm = elem.xpath('./s2:HashAlgorithm',
                                   namespaces=S2_NS)[0].text
        hashvalue = elem.xpath('./s2:HashValue', namespaces=S2_NS)[0].text
        creation_date = import_object.creation_date(filepath)
        ...
        create_premis_object(filename, package_path, formatname,
...
def formatdesignation(filepath, datatype=None):
    ...
    # If it's plain text, return mimetype and charset
    if formats[1] == 'text,':
        charset_raw = str(subprocess.Popen(
            command_bi,
            stdout=subprocess.PIPE).stdout.read()).rsplit(None, 1)[-1]
        print charset_raw
        if charset_raw == 'charset=us-ascii':
            charset = 'charset=ISO-8859-15'
        elif charset_raw == 'charset=iso-8859-1':
            charset = 'charset=ISO-8859-15'
        else:
            charset = charset_raw
        formatname = subprocess.Popen(
            command_bi,
            stdout=subprocess.PIPE).stdout.read().rsplit(';', 1)[0] + '; ' + charset
        formatversion = ''
