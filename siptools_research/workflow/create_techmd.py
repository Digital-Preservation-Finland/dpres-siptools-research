...
class CreateTechnicalMetadata(WorkflowTask):
    ...
    def run(self):
        ...
        # Create one timestamp for import_object events to avoid creating
        # new events each time import_object is iterated
    ...
    def create_objects(self, metadata, event_datetime):
        """Create PREMIS metadata for file. Reads file metadata from Metax.
        Technical metadata is generated by siptools import_object script.

        :param metadata: file metadata dictionary
        :param event_datetime: the timestamp for the import_object events
        :returns: ``None``
        """
    ...
    def create_technical_attributes(self, metadata):
        """Read technical attribute XML from Metax. Create METS TechMD files
        for each metadata type, if it is available in Metax.
