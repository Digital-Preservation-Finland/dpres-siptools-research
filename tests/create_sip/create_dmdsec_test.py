"""Test the :mod:`siptools_research.create_sip.create_dmdsec` module"""
...
def test_create_dmdsec_ok(testpath, tarfile):
    """Test the workflow task CreateDescriptiveMetadata.
    """
    ...
    task = CreateDescriptiveMetadata(workspace=workspace,
            sip_creation_path=sip_creation_path,
                home_path='/home')

    assert task_ok(task)
    assert task.complete()
    assert os.path.isfile(os.path.join(sip_creation_path, 'dmdsec.xml'))
...
def assert_mongodb_data_success(document_id):
    ...
    mongodb_data = collection.find({"_id": document_id},
            {"wf_tasks.create-descriptive-metadata.result": 1})

    for item in mongodb_data:
        assert item["wf_tasks"]["create-descriptive-metadata"]["result"] == "success"
