"""Test the :mod:`siptools_research.create_sip.create_mets` module"""
...
from siptools_research.create_sip.create_mets import CreateMets, \
        get_s2_nativeid
from siptools_research.create_sip.create_structmap import CreateStructMap
...
def test_create_mets_ok(testpath, tarfile):
    ...
    create_digiprov = CreateProvenanceInformation(workspace=workspace,
            sip_creation_path=sip_creation_path,
                home_path='/home')

    task_ok(create_digiprov)

    create_techmd = CreateTechnicalMetadata(workspace=workspace,
            sip_creation_path=sip_creation_path,
                home_path='/home')

    task_ok(create_techmd)

    create_structmap = CreateStructMap(workspace=workspace,
                    sip_creation_path=sip_creation_path,
                                    home_path='/home')

    assert task_ok(create_structmap)
    ...
    assert os.path.isfile(os.path.join(sip_creation_path, 'mets.xml'))
...
def test_get_s2_nativeid_ok(testpath):
    """Tests the get_s2_nativeid function."""

    sahke2file = 'tests/data/transfer/valid_transfer/sahke2.xml'

    nativeid = get_s2_nativeid(sahke2file)
