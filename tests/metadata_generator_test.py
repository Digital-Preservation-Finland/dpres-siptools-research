...
from tests.metax_data.files import FILE
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "CR",
            "File scraper detects a different csv_record_separator: None",
            "tests/data/sample_files/text_plain_UTF-8"
        ),
        (
            "csv_delimiter",
            "foo",
            "File scraper detects a different csv_delimiter: None",
            "tests/data/sample_files/text_plain_UTF-8"
        ),
        (
            "csv_quoting_char",
            "foo",
            "File scraper detects a different csv_quoting_char: None",
            "tests/data/sample_files/text_plain_UTF-8"
        ),
        ...
        #     "encoding",
        #     "foo",
        #     "File scraper detects a different encoding: UTF-8",
        #     "tests/data/sample_files/text_plain_UTF-8"
        # )
    ]
)
...
def test_overwriting_user_defined_metadata(config, requests_mock, tmp_path,
                                           key, predefined_value,
                                           expected_error, filepath):
    ...
    :param requests_mock: Mocker object
    :param tmp_path: Temporary directory
    :param key: key to be modified in file characteristics
    ...
    # characteristics
    file = copy.deepcopy(FILE)
    file["characteristics"][key] = predefined_value
    dataset = add_metax_dataset(requests_mock, files=[file])
    requests_mock.get("/v3/reference-data/file-format-versions",
                      json=tests.metax_data.reference_data.FILE_FORMAT_VERSIONS)
