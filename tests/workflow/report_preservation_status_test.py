...
def test_reportpreservationstatus_rejected(testpath, testmongoclient,\
                                           testmetax):
    ...
    with paramiko.SSHClient() as ssh:
        ...
        datedir = time.strftime("%Y-%m-%d")
        tar_name = os.path.basename(workspace) + '.tar'
        ssh.exec_command("mkdir -p rejected/%s/%s" % (datedir, tar_name))

    # Init and run task
    with mock.patch('siptools_research.workflow.report_preservation_status.mail.send') as mock_sendmail:
